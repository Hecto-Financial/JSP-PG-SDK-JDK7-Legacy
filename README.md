...
> **레거시 전용 경고**
>
> JDK 7과 Tomcat 7은 지원이 종료된 런타임입니다. 이 샘플은 기존 가맹점의
> 불가피한 호환을 위해서만 제공하며 신규 구축에는 사용하지 마십시오.
> 가능한 빠르게 지원 중인 최신 LTS JDK와 WAS로 전환해야 합니다.

> **운영 로그 주의**
>
> 샘플에는 연동 확인을 위한 요청·응답 및 암호화 대상 값 로그가 포함되어 있습니다.
> 운영 배포 전 개인정보, 결제정보, 키 관련 값이 기록되지 않도록 반드시 제거하거나
> 마스킹하고 가맹점의 로그 보관·접근 정책을 적용하십시오.

## TLS 보안

`HttpClientUtil`은 `TLSv1.2`만 활성화합니다. JRE 기본 TrustStore를 사용해 서버 인증서
체인을 검증하고 `HttpsURLConnection`의 기본 호스트명 검증을 유지합니다. 모든 인증서나
호스트명을 신뢰하는 우회 설정은 사용하지 않습니다.
