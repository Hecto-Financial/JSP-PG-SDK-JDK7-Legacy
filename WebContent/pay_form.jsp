...
function init(type){
    ...
    $('#STPG_payForm [name="custIp"]').val("<%= request.getRemoteAddr() %>"); //고객 IP 세팅
...
function autoPay(){
    init('mobileAuto');
    $('#STPG_payForm2').attr("action", "pay_autoPayResult.jsp");
...
/** pay_receiveResult.jsp로부터 응답 값을 받아와서 main폼에 세팅 */
...
/** main폼에 세팅된 응답 값 출력 */
function goResult(){
    $('#STPG_payForm').attr("action", "pay_showResult.jsp");
...
<div class="wrapper">  
    <form id="STPG_payForm" name="STPG_payForm" >
        ...
        <input type="hidden" name="cancUrl" value="http://localhost/pay_receiveResult.jsp" />   <!-- 결제취소 URL -->
        ...
        <table>
            <tr>
                ...
                <td class="right"><input type="text" name="mchtId" value="<%= PG_MID %>" /></td>
            </tr>
            <tr>
