...
REQ_HEADER.put("bizType",   StringUtil.isNull(request.getParameter("bizType")));    //업무구분(S1:빌키 상태조회, C1:빌키 삭제)
...
String bizType = REQ_HEADER.get("bizType");
String apiPath = "S1".equals(bizType) ? "/spay/APIPZSubsStatus.do" : "/spay/APIPZSubsDelKey.do";
String jobName = "S1".equals(bizType) ? "빌키 상태조회" : "빌키 삭제";
...
String hashCipher="";
try{
    hashPlain = String.format("%s%s%s%s%s"
...
//URL설정
String requestUrl = apiHost + apiPath; //간편 정기결제 빌키 상태조회/삭제 URL
...
<title>간편 정기결제 <%= jobName %> 결과</title>
<style type="text/css">
